...
import calendarValues from './assets/calendar-values.json'
...
import {faCoffee, faTimes, faBars, faBell, faFileMedicalAlt, faInfo, faListAlt, faCalendarAlt} from '@fortawesome/free-solid-svg-icons'

import LiveAlarms from './components/graphical_views/AlarmsComponent'
import vuetify from './plugins/vuetify';

library.add(faCoffee, faTimes, faBars, faBell, faFileMedicalAlt, faInfo, faListAlt, faCalendarAlt)
...
if (window.document.getElementById('app') != undefined) {
  new Vue({
    i18n,
    router,
    store,
...
window.initCalendar = function(componentId, props) {
  props.calendarValues = calendarValues;
  if (!window.vueComponents) window.vueComponents = [];
  window.vueComponents.push(new Vue({
    i18n,
    store,
    vuetify,
    el: '#c'+componentId+'ContentBody',
    render: h => h(CalendarComponent, {props: {componentId,
      ...props}})
  }));
};
