...
public class ImageChartUtils {
    ...
    public static void writeChart(PointTimeSeriesCollection pointTimeSeriesCollection, boolean showLegend,
        ...
        // Get max time stamps across all numeric and discrete series        
        long highestVal = 0;
        
        if(pointTimeSeriesCollection.hasNumericData()) {
        	for (int i = 0; i < pointTimeSeriesCollection.getNumericTimeSeriesCollection().getSeriesCount(); i++) {
        		TimeSeries series = pointTimeSeriesCollection.getNumericTimeSeriesCollection().getSeries(i);
        		if(series.getItemCount() > 0) {
        			long highestTS = series.getDataItem(series.getItemCount() - 1).getPeriod().getFirstMillisecond();
        			if(highestTS > highestVal)
        				highestVal = highestTS;
        		}
        	}
        }
        ...
        if (pointTimeSeriesCollection.hasDiscreteData()) {
        	for (int i = 0; i < pointTimeSeriesCollection.getDiscreteSeriesCount(); i++) {
                ...
                if(dts.getValueTimes().size() > 0) {
                	if(dts.getValueTimes().get(dts.getValueTimes().size() - 1).getTime() > highestVal)
                		highestVal = dts.getValueTimes().get(dts.getValueTimes().size() - 1).getTime();                	                	
                }
            }
        }
        ...
        if (pointTimeSeriesCollection.hasNumericData()) {
        	for (int i = 0; i < pointTimeSeriesCollection.getNumericTimeSeriesCollection().getSeriesCount(); i++) {
        		TimeSeries series = pointTimeSeriesCollection.getNumericTimeSeriesCollection().getSeries(i);
        		if(series.getItemCount() > 0) {
        			ImageChartUtils.addSecond(series, highestVal, series.getValue(series.getItemCount() - 1));
        		}        		
        	}
        ...
        if (pointTimeSeriesCollection.hasDiscreteData()) {
            ...
            for (int i = 0; i < pointTimeSeriesCollection.getDiscreteSeriesCount(); i++) {
                DiscreteTimeSeries dts = pointTimeSeriesCollection.getDiscreteTimeSeries(i);                
                if(dts.getValueTimes().size() > 0) {
                	PointValueTime pvt = new PointValueTime(dts.getValueTimes().get(dts.getValueTimes().size() - 1).getValue(), highestVal);
                	dts.addValueTime(pvt);
                }
            }
