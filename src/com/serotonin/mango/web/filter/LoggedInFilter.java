...
abstract public class LoggedInFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        if (!loggedIn) {
            ...
            request.getSession().setAttribute("url_prior_login", getRequestUri(request));
    ...
    private String getRequestUri(HttpServletRequest request) {
    	...
    	uriWithContext = uriWithContext.replace(ctxPath, "").trim();
    	if(uriWithContext.startsWith("/") && uriWithContext.trim().length() > 1)
    		uriWithContext = uriWithContext.substring(1);
    	
        if(request.getQueryString() != null)
			uriWithContext = uriWithContext + "?" + request.getQueryString();
		return uriWithContext;
    }
