...
import com.serotonin.mango.view.component.AnalogCompoundComponent;
...
import com.serotonin.mango.view.component.CalendarComponent;
...
public class ViewDwr extends BaseDwr {
	...
	public List<IntValuePair> getViews() {
		...
		return viewDao.getViewNames(user.getId(), user.getUserProfile());
	}
	...
	private List<ViewComponentState> getViewPointData(User user, View view, boolean edit) {
		...
		if (view == null || view.getViewComponents() == null)
			return states;
		for (ViewComponent viewComponent : view.getViewComponents()) {
			if (viewComponent.isCompoundComponent() && (edit || viewComponent.isVisible())) {
				...
				boolean analogCompound = compoundComponent instanceof AnalogCompoundComponent;

				if (analogCompound) {
					ViewComponentState state = prepareAnalogCompountComponentState(
							(AnalogCompoundComponent) compoundComponent, user, rtm, model, request);
					states.add(state);
				} else {
					...
					for (CompoundChild child : compoundComponent.getChildComponents())
						addPointComponentState(child.getViewComponent(), rtm, model, request, view, user, states, edit,
								!imageChart && !enhancedImageChart && !analogCompound);
	...
	private void addPointComponentState(ViewComponent viewComponent, RuntimeManager rtm, Map<String, Object> model,
		...
		if (viewComponent.isPointComponent() && (edit || viewComponent.isVisible())) {
			...
			if (dataPointRT == null || Permissions.hasDataPointReadPermission(user, dataPointRT.getVO())) {
				ViewComponentState state = preparePointComponentState(pointComponent, user, dataPointRT, model,
						request);

				if (!edit) {
					if (pointComponent.isSettable() && dataPointRT != null
							&& Permissions.hasDataPointSetPermission(user, dataPointRT.getVO())) {
	...
	private ViewComponentState prepareAnalogCompountComponentState(AnalogCompoundComponent compoundComponent, User user,
			RuntimeManager rtm, Map<String, Object> model, HttpServletRequest request) {
		ViewComponentState state = new ViewComponentState();
		state.setId(compoundComponent.getId() + "-point1");
		if (!compoundComponent.isValid())
			model.put("invalid", "true");
		else
			model.put("image", compoundComponent.getImage(rtm));
		state.setContent(generateContent(request, "imageContent.jsp", model));
		state.setInfo(generateContent(request, "infoContent.jsp", model));
		setMessages(state, request, "warningContent", model);
	...
	@MethodFilter
	public void addMapData(Object mapData) {
		User user = Common.getUser();
		View view = user.getView();
	...
	@MethodFilter
	public DwrResponseI18n saveCalendarComponent(String viewComponentId, String content) {
		CalendarComponent c = (CalendarComponent) getViewComponent(viewComponentId);
		c.setContent(content);
	...
	@MethodFilter
	public DwrResponseI18n saveAnalogCompoundComponent(String viewComponentId, String name, String backgroundColour,
			List<KeyValuePair> childPointIds) {
		DwrResponseI18n response = new DwrResponseI18n();
	...
	@MethodFilter
	public DwrResponseI18n saveEnhancedImageChartComponent(String viewComponentId, String name, int width, int height,
			int durationType, int durationPeriods, EnhancedImageChartType chartType, List<KeyValuePair> childPointIds,
			List<EnhancedPointComponentProperties> pointsPropsList) {
	...
	@MethodFilter
	public DwrResponseI18n saveAlarmListComponent(String viewComponentId, int minAlarmLevel, String messageContent,
			int maxListSize, int width, boolean hideIdColumn, boolean hideAlarmLevelColumn, boolean hideTimestampColumn,
			boolean hideInactivityColumn, boolean hideAckColumn, boolean hideCriteriaHeader) {
		DwrResponseI18n response = new DwrResponseI18n();
		// Validate
	...
	private void commonImageChartComponentValidation(String name, int width, int height, int durationType,
			int durationPeriods, DwrResponseI18n response) {
		validateCompoundComponent(response, name);
		if (width < 1)
	...
	private void validateEnhancedPoints(List<EnhancedPointComponentProperties> pointsPropsList,
		...
		for (EnhancedPointComponentProperties props : pointsPropsList) {
			...
			if (props.getStrokeWidth() < 0) {
				response.addContextualMessage("compoundPointStrokeWidth" + props.getKey(),
