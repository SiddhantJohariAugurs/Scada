		...
		while (!pollRequest.isTerminated()
			...
			if (pollRequest.isMaxAlarm() && user != null) {
				...
				if (state.getLastAlarmLevelChange() < lastEMUpdate) {
					...
					int minAlarmLevel = AlarmLevels.URGENT;

					// Disable alram only if level is greater than or equal to minimum alarm level
					if (maxAlarmLevel != state.getMaxAlarmLevel() && maxAlarmLevel >= minAlarmLevel) {
	...
	public List<EventInstance> convertTime(List<EventInstance> events, User user) {
		events.forEach(item-> {
			if(!user.ids.contains(item.getId())) {
				...
				if(item.getAlarmLevel() >= AlarmLevels.URGENT)
