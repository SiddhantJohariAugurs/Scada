...
public class EmailHandlerRT extends EventHandlerRT implements ModelTimeoutClient<EventInstance> {
    ...
    public void eventRaised(EventInstance evt) {
        ...
        if (vo.isSendEscalation() && !evt.isAcknowledged()) {
    ...
    synchronized public void scheduleTimeout(EventInstance evt, long fireTime) {
        ...
        // Send the escalation.
        EventInstance updatedEvt = eventService.getById(evt.getId());
        if(!updatedEvt.isAcknowledged())
    ...
    private static void sendEmail(EventInstance evt, NotificationType notificationType, Set<String> addresses,
        ...
        try {
            ...
            model.put("ackUri", getAckUrl(evt.getId()));
    ...
    private static String getAckUrl(int eventId) {
		if (WebContextFactory.get() != null && WebContextFactory.get().getHttpServletRequest() != null) {
            HttpServletRequest request = WebContextFactory.get().getHttpServletRequest();
            LOG.info("Server name: " + request.getServerName());
			return String.format("%s://%s:%d%s/alarm_ack.shtm?alarmId=", request.getScheme(), request.getServerName(),
					request.getServerPort(), request.getContextPath()) + eventId;
		} else {
			return "https://webscada.cob.vroc.ai/ScadaLTS/alarm_ack.shtm?alarmId=" + eventId;
		}
	}
}
