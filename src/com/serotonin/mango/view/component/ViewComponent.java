...
abstract public class ViewComponent implements Serializable, JsonSerializable {
	...
	public static List<ImplDefinition> getImplementations() {
		if (DEFINITIONS == null) {
			...
			d.add(AnalogCompoundComponent.DEFINITION);
			d.add(CalendarComponent.DEFINITION);
	...
	static Class<? extends ViewComponent> resolveClass(ImplDefinition def) {
		...
		if (def == AnalogCompoundComponent.DEFINITION)
            return AnalogCompoundComponent.class;
		if (def == CalendarComponent.DEFINITION)
