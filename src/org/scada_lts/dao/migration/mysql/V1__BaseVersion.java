...
public class V1__BaseVersion implements SpringJdbcMigration {
	...
	public void migrate(JdbcTemplate jdbcTemplate) throws Exception {
      ...
      //System settings
       ...
       //Create Layers
	    final String layersSql = ""
            		+ "create table layers ("
            			+ "layerId int not null auto_increment,"
            			+"layerName varchar(50) not null,"
            			+ "layerDescription varchar(1000) not null,"
    						+ "primary key (layerId)"
            		+ ") ENGINE=InnoDB;";

       jdbcTemplate.execute(layersSql);
 String sqlLayerAdd =
		   "insert into layers (" +
			   "layerName, " +
		   ...
		   "values (?,?)";
		      ...
		      jdbcTemplate.update(sqlLayerAdd, new Object[]{
	   	"Default Layer",
	   ...
	   jdbcTemplate.update(sqlUserAdd, new Object[]{
	   		"admin",
			Common.encrypt("admin"),
			"admin@yourDomain.com",
			"",
			DAO.boolToChar(true),
			DAO.boolToChar(false),
			"",
			new Integer(0),
			DAO.boolToChar(false)
	   });
