...
public class DataPointDAO {
	...
	public List<DataPointVO> getSchedule(Integer dataSourceId, String startPrefix, String endPrefix, String valuePrefix) {

		if (LOG.isTraceEnabled()) {
			LOG.trace("getSchedule(String dataSourceId, String startPrefix, String endPrefix, String valuePrefix) dataSourceId:" + dataSourceId + " startPrefix: " + startPrefix + " endPrefix: " + endPrefix + " valuePrefix: " + valuePrefix);
		}

		String template = DATA_POINT_SELECT + " WHERE dp." + COLUMN_NAME_DATA_SOURCE_ID + " = ? AND (dp." + COLUMN_NAME_DATAPOINT_NAME + " LIKE ? OR dp." + COLUMN_NAME_DATAPOINT_NAME + " LIKE ? OR dp." + COLUMN_NAME_DATAPOINT_NAME + " LIKE ?)";

		try {
			return DAO.getInstance().getJdbcTemp().query(template, new Object[] {dataSourceId, startPrefix+'%', endPrefix+'%', valuePrefix+'%'}, new DataPointRowMapper());
		} catch (Exception e) {
