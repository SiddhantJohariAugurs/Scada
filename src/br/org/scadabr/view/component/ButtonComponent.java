...
public class ButtonComponent extends ScriptComponent {
	...
	private void createButtonScript() {
		...
		if (width > 0 && height > 0) {
			sb
					.append(" s += \"<button onclick='if(confirm(\\\"Are you sure to perform this action?\\\")) { mango.view.setPoint(\"+ point.id +\",\"+ pointComponent.id +\", false);return false;}' style='width:"
							+ width + "px; height:" + height + "px;'>" + whenOnLabel + "</button>\";");
			sb.append(" else");
			sb
					.append(" s += \"<button onclick='if(confirm(\\\"Are you sure to perform this action?\\\")) { mango.view.setPoint(\"+ point.id +\",\"+ pointComponent.id +\", true);return true;}' style='width:"
							+ width + "px; height:" + height + "px;'>" + whenOffLabel + "</button>\";");
		} else {
			sb
					.append(" s += \"<button onclick='if(confirm(\\\"Are you sure to perform this action?\\\")) { mango.view.setPoint(\"+ point.id +\",\"+ pointComponent.id +\", false);return false;}'>" + whenOnLabel + "</button>\";");
			sb.append(" else");
			sb
					.append(" s += \"<button onclick='if(confirm(\\\"Are you sure to perform this action?\\\")) { mango.view.setPoint(\"+ point.id +\",\"+ pointComponent.id +\", true);return true;}'>" + whenOffLabel + "</button>\";");
		}
