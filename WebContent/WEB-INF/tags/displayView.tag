...
<div id="viewContent">
  ...
  <c:forEach items="${view.viewComponents}" var="vc">
    ...
    <c:choose>
      ...
      <c:when test="${vc.defName == 'calendar'}">
        <div id="c${vc.id}" class="calendar" style="position:absolute;left:${vc.x}px;top:${vc.y}px;">
          <div id="c${vc.id}Content" class="calendar-content"><div id="c${vc.id}ContentBody" class="calendar-body"></div></div>
        </div>
        <script>
       	  var data = {};
          try {
        	  data = JSON.parse('${vc.content}');
          } catch(e) {  
          }
          window.initCalendar(${vc.id},data);
        </script>
      </c:when>
      ...
      <c:when test="${vc.defName == 'analogCompound'}">
      	<div id="c${vc.id}" style="position:absolute;left:${vc.x}px;top:${vc.y}px;"
                  onmouseover="vcOver('c${vc.id}', 5);" onmouseout="vcOut('c${vc.id}');">
          <c:forEach items="${vc.childComponents}" var="child">
            <c:if test="${child.viewComponent.visible && child.id == 'point1'}"><tag:pointComponent vc="${child.viewComponent}"/></c:if>
          </c:forEach>
        </div>
      </c:when>
