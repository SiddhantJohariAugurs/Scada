...
<% request.setAttribute("defaultAlarmLevel", Integer.toString(AlarmLevels.URGENT)); %>
<tag:page dwr="EventsDwr">
  ...
  <script type="text/javascript" src="resources/jQuery/plugins/jquery-ui/js/jquery-ui-1.10.3.custom.min.js"></script>
...
//     dojo.requireLocalization("dojo.i18n.calendar", "gregorianExtras", null, "ROOT,ja,zh");
  	...
  	jQuery(function() {
		...
		jQuery('#alarmLevel').val("${defaultAlarmLevel}");
	});
    ...
    function acknowledgeEvents() {
    	var eventIds = [];
    	jQuery("#searchResults table tbody tr.row,tr.rowAlt").each(function(){
    		ackEvent(jQuery(this).find("td:first").text());
      });
      doSearchOld();
    }
    
    function doSearchOld() {
    	mango.longPoll.pollRequest.pendingAlarms = false;
    	hide("pendingAlarmsMain");    	
    	setDisabled("searchBtn", true);
    	$set("searchMessage", "<fmt:message key="events.search.searching"/>");
		EventsDwr.searchOld($get("eventId"), $get("eventSourceType"), $get("eventStatus"), $get("alarmLevel"), $get("keywords"), $get("maxResults"), function(results) {
			$set("searchResults", results.data.content);
			setDisabled("searchBtn", false);
			$set("searchMessage", results.data.resultCount);
			show("ackAllSearchResultDiv");			
		});
	}
        ...
        <tr>
          ...
          <td class="formField">
            <select id="eventStatus">
              ...
              <option value="<c:out value="<%= EventsDwr.STATUS_NO_ACK %>"/>"><fmt:message key="common.noack"/></option>
            </select>
          </td>
        </tr>
        <tr>
          <td class="formLabel"><fmt:message key="common.minimumAlarmLevel"/></td>
...
<!--         </tr> -->
        ...
        <tr>
        	<td class="formLabel">&nbsp;</td>
          	<td class="formField"><input id="searchBtn" type="button" value="<fmt:message key="events.search.search"/>" onclick="newSearch()"/><span id="searchMessage" class="formError"></span></td>          
        </tr>
                
      </table>
    </div>
    <div id="ackAllSearchResultDiv" class="titlePadding" style="display:none;float:right;">
      <fmt:message key="events.acknowledgeAll"/>
      <tag:img png="tick" onclick="acknowledgeEvents()" title="events.acknowledgeAll"/>&nbsp;
      <fmt:message key="events.silenceAll"/>
      <tag:img png="sound_mute" onclick="silenceAll()" title="events.silenceAll"/><br/>
    </div>
    <div id="searchResults"></div>
  </div>
  <div style="clear:left;width:50%; height:10px"></div>
  <div id="pendingAlarmsMain" class="borderDiv marB" style="clear:left;float:left;">
